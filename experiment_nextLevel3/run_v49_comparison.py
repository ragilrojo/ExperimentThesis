...
def run_simulation(test_dates, returns, ai_probs, threshold, fee=0.0025, g_lookback=60, rebal_period=20, turnover_buffer=0.05):
    val = 100.0
    history = [val]
    dates = [test_dates[0]]
    current_weights = {} 
    ...
    for i, date in enumerate(test_dates[:-1]):
        ...
        if not risk_status:
            ...
            if i == 0: is_time_to_rebal = True # Force initial invest for static
...
print(f"--- Simulation (Fee={FEE*100}%, Rebal={PERIOD}d, Buffer={BUFFER*100}%) ---")

# 1. V48 AI Smart
res_ai = run_simulation(test_dates, returns, test_probs, opt_t, fee=FEE, rebal_period=PERIOD, turnover_buffer=BUFFER)
print(f"V48 (AI Smart): {res_ai:.2f}")

# 2. Static Markowitz
res_static = run_simulation(test_dates, returns, test_probs, -1.0, fee=FEE, rebal_period=PERIOD, turnover_buffer=BUFFER)
print(f"Static Markowitz: {res_static:.2f}")
