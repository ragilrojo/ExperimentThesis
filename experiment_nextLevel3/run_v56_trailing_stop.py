...
def run_simulation_trailing_stop(test_dates, returns, ai_probs, threshold, 
    ...
    for i, date in enumerate(test_dates[:-1]):
        ...
        if not risk_status:
            # Need strict Bull Signal to enter
             if prob > (threshold + 0.05):
                new_status = True
                high_watermark = val # Reset HWM on Re-Entry
...
# For Static simulation, assuming standard behavior, we store it. Note: Static has NO AI Exit logic, so it never exits.
results['Static Markowitz'] = res_static['Portfolio_Value']

print("Running Trail Stop 5% (Ignore AI Bear)...")
# Note: For Trail Stop strategies, we rely on AI only for ENTRY. Once in, only TS exits.
# But wait, run_simulation logic needs to start.
# If AI Probs on day 1 is low, we start in Cash.
# We need to make sure we enter. Opt Threshold is usuall around 0.5.
...
print("Saved plot to v56_trailing_stop_results.png")
