...
notebook["cells"].append(create_md_cell([
    ...
    "\n",
    "### Comparisons:\n",
    "1. **AI Hybrid V44 (Cost-Sensitive)**.\n",
...
notebook["cells"].append(create_code_cell([
    ...
    "# We can also increase class weight for '0' (Exit) slightly if we want to be careful,\n",
    "# but shifting the threshold itself is usually enough.\n",
    ...
    "\n",
    "# With this new target definition, 'Bullish' means 'Not Crashed'.\n",
    "# So a high probability means 'Safe to Hold'.\n",
...
# Cell 4: Functions
notebook["cells"].append(create_code_cell([
    "# --- 3. Simulation Logic (Cost Sensitive) ---\n",
    "\n",
    ...
    "            # prob is now P(Return > -0.5%)\n",
    "            # So if prob is high, it means we are safe.\n",
    "            # We exit if Prob is LOW (< T) AND Trend Broken\n",
...
notebook["cells"].append(create_code_cell([
    ...
    "hybrid_res_cost = run_simulation_cost_aware('AI Hybrid V44 (CostAware)', X_test.index, returns, ai_probs, test_trend, threshold=0.50)\n",
    "\n",
    "print(\"Running Markowitz Static...\")\n",
    "static_res = run_simulation_cost_aware('Markowitz Static', X_test.index, returns)\n",
    "\n",
