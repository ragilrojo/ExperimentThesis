...
def run_simulation_trend_override(test_dates, returns, ai_probs, threshold, market_idx, 
                                  override_ma=None, # e.g., 50, 100, 200
                                  fee=0.0025, 
                                  g_lookback=60, 
                                  rebal_period=20, 
    ...
    # Calculate MA for Trend Override
    market_ma = None
    if override_ma:
    ...
    for i, date in enumerate(test_dates[:-1]):
        ...
        if override_ma and market_ma is not None:
            # Handle possible missing MA at start of period due to rolling
            # Use LOC with safety or forward fill approach if needed, but since we have long history it should be fine
            # Actually, market_idx starts way back?
            # Returns starts 2023. X_test starts 2025.
            # So rolling 200 should be available by 2025.
            
            current_price = market_idx.loc[date]
            # get MA value. If NaN (start of data), default to False override
            try:
...
# 2. Trend Override AI (SMA 50) - Baseline V52
...
# 3. Trend Override AI (SMA 100)
print("Running Trend Override AI (SMA 100)...")
res_trend100 = run_simulation_trend_override(test_dates, returns, test_probs, opt_t, market_index, 
                                            override_ma=100, fee=fee_rate)
results['AI + Trend Override (SMA100)'] = res_trend100['Portfolio_Value']

# 4. Trend Override AI (SMA 200)
print("Running Trend Override AI (SMA 200)...")
res_trend200 = run_simulation_trend_override(test_dates, returns, test_probs, opt_t, market_index, 
                                            override_ma=200, fee=fee_rate)
results['AI + Trend Override (SMA200)'] = res_trend200['Portfolio_Value']
...
plt.title('V53: Advanced Trend Override (SMA 100/200) vs Static Strategy')
plt.ylabel('Portfolio Value')
plt.legend()
plt.grid(True, alpha=0.3)

plt.savefig('v53_advanced_trend_override.png')
print("Saved plot to v53_advanced_trend_override.png")
...
print("\n=== Performance ROI & Risk (V53) ===")
print(df_summary.to_string())

df_results = pd.DataFrame(results)
df_results.to_excel('v53_advanced_trend_override_results.xlsx', index=False)
print("Saved results to v53_advanced_trend_override_results.xlsx")
