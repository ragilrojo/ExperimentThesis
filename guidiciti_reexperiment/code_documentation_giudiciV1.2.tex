...
class AdaptiveGraphPortfolio(PortfolioStrategy):
    """
    AGGP v1.2 (Fair V9): Memperbaiki bug sensor pada V9 dengan korelasi riil,
    namun menjaga profil low-risk dengan membatasi gamma_max = 0.5.
    """
    ...
    def get_weights(self, r):
        ...
        try:
            ...
            # FIX 1: Konversi Covariance ke Korelasi agar sensor responsif
            # V9 asli menggunakan covariance mentah yang nilainya sangat kecil
            v = np.sqrt(np.diag(Sf)); inv_sig = np.diag(1.0 / (v + 1e-8))
            Cf = inv_sig @ Sf @ inv_sig
        except:
            Cf = apply_rmt_filter(r); Sf = np.outer(sig, sig) * Cf

        # FIX 2: Sensor Density pada korelasi riil (> 0.5)
        adj = (np.abs(Cf) > 0.5).astype(int); G = nx.from_numpy_array(adj)
        density = nx.density(G)
        
        # FIX 3: Atur sensitivitas & gamma_max (Low Risk Profile)
        # Portofolio tetap defensif (dominan Markowitz) demi profil risiko rendah.
