...
\begin{enumerate}
    ...
    \item \textbf{Adaptive Graph-Gated Portfolio (AGGP v3)}: Model usulan (\textit{Proposed Method}) yang menggabungkan optimasi Markowitz (\textit{Global Minimum Variance}) dengan komponen Network Markowitz ($\gamma=1.0$). Proporsi campuran ($\gamma_t$) bersifat \textbf{adaptif}: dikendalikan oleh sensor \textit{average partial correlation} yang proper --- dihitung dari matriks presisi Graphical Lasso menggunakan rumus $\rho^{\text{partial}}_{ij} = -P_{ij}/\sqrt{P_{ii} P_{jj}}$, dijamin berada pada rentang $[0,1]$. Mekanisme switching gabungan \textit{threshold} eksplisit dan \textit{soft blending} membuat AGGP benar-benar adaptif: saat pasar stres ($\bar{\rho}^{p} \geq \tau$) otomatis masuk mode defensif; saat normal ($\bar{\rho}^{p} < \tau$) memanfaatkan struktur jaringan secara agresif.
\end{enumerate}
...
%=============================================================================
Setiap strategi diatur dalam kelas yang mewarisi \texttt{PortfolioStrategy}. Implementasi \textit{Network Markowitz} mencakup penalti sentralitas melalui parameter $\gamma$. Kelas \textbf{AGGP v3} (\textit{Adaptive Graph-Gated Portfolio}) merupakan model usulan yang diperbarui dengan tiga perbaikan kritis terhadap versi sebelumnya:

\begin{table}[h]
    \centering
    \footnotesize
    \caption{\textbf{Perbaikan Kritis AGGP v3 terhadap AGGP v2.}}
    \begin{tabular}{p{0.8cm} p{4.5cm} p{8.5cm}}
        \toprule
        \textbf{Fix} & \textbf{Masalah} & \textbf{Solusi} \\
        \midrule
        \textbf{Fix A} &
        Sensor \texttt{avg\_corr} dari \texttt{Pf} mentah $\rightarrow$ nilai tidak terkontrol (bisa $> 10$, menyebabkan \texttt{gamma\_t} selalu terpotong di 0.1) &
        Hitung \textbf{korelasi parsial proper}: $\rho^p_{ij} = {-P_{ij}}/\sqrt{P_{ii} P_{jj}}$, lalu ambil rata-rata nilai absolutnya. Hasilnya dijamin pada rentang $[0, 1]$. \\
        \midrule
        \textbf{Fix B} &
        \texttt{gamma\_t} hampir selalu di batas bawah 0.1, tidak ada variasi adaptif nyata &
        Dengan sensor $[0,1]$, rumus \texttt{gamma\_t = 1 - avg\_pcorr} bekerja natural tanpa parameter \texttt{sensitivity} yang perlu dikalibrasi manual. \\
        \midrule
        \textbf{Fix C} &
        Tidak ada mekanisme \textit{``momentum switching''} --- transisi antara mode defensif dan agresif terlalu halus &
        Tambahkan \textbf{threshold eksplisit} $\tau$: jika $\bar{\rho}^p \geq \tau$ $\rightarrow$ \textit{defensive mode} (Markowitz murni); jika $\bar{\rho}^p < \tau$ $\rightarrow$ \textit{soft blending} agresif ke NW $\gamma = 1$. \\
        \bottomrule
    \end{tabular}
\end{table}

\begin{lstlisting}[caption={Kelas-kelas strategi portofolio (AGGP v3 -- Fix A/B/C Applied)}]
...
class AdaptiveGraphPortfolio(PortfolioStrategy):
    ...
    dijamin berada di rentang [0, 1]. Mekanisme switching menggunakan
    threshold eksplisit + soft blending.
    ...
    Mekanisme adaptasi gamma_t (FIX A + FIX B + FIX C):
      - avg_pcorr >= stress_threshold (pasar stres) -> gamma_t = gamma_min
        -> portofolio DOMINAN Markowitz (defensif, minimasi varians)
      - avg_pcorr < stress_threshold (pasar normal)  -> gamma_t soft-blended
        -> portofolio lebih ke Network Markowitz gamma=1.0 (agresif)

    Komponen w_net: NW gamma=1.0 (best performer Giudici et al. 2020)
    Fallback aman: EW jika optimizer gagal konvergen.
    """
    def __init__(self, name="AGGP v3", stress_threshold=0.25,
                 gamma_min=0.1, gamma_max=0.9):
        super().__init__(name)
        self.stress_threshold = stress_threshold  # batas deteksi pasar stres
        self.gamma_min        = gamma_min          # bobot NW saat stres (defensif)
        self.gamma_max        = gamma_max          # bobot NW saat normal (agresif)
    ...
    def get_weights(self, returns_data):
        ...
        # --- KOMPONEN NETWORK: NW gamma=1.0 (best performer Giudici et al.) ---
...
\begin{itemize}
    \item Saat \textbf{Bearish}: sensor \textit{avg\_pcorr} (korelasi parsial proper) mendeteksi nilai $\geq \tau$ $\Rightarrow$ \textit{threshold switching} aktif $\Rightarrow$ $\gamma_t = \gamma_{\min}$ (hampir murni Markowitz, defensif).
    \item Saat \textbf{Recovery/Stable}: $\overline{\rho}^p < \tau$ $\Rightarrow$ \textit{soft blending} aktif $\Rightarrow$ $\gamma_t$ naik mendekati $\gamma_{\max}$ $\Rightarrow$ AGGP memanfaatkan struktur jaringan NW $\gamma=1.0$ secara agresif.
\end{itemize}
Hipotesis yang diuji: AGGP v3 seharusnya mengalahkan NW($\gamma=1.0$) pada fase \textit{Bearish} (berkat mode pertahanan yang lebih kuat) dan bersaing setara atau lebih baik pada fase \textit{Recovery/Stable}, karena sensor korelasi parsial proper kini benar-benar bervariasi pada rentang $[0,1]$ dan tidak selalu terpotong di batas bawah.
