...
# -- 9. Optimized Grid AGGP (Deep Learning Level 2) --------------------------
...
\textbf{Optimasi Parameter Dinamis (Grid Search):}
Sebagai pengembangan lebih lanjut dari model AGGP statis, varian \textit{Grid Search} diperkenalkan untuk menentukan konfigurasi parameter gating secara otomatis. Model ini melakukan evaluasi \textit{Hyperparameter} menggunakan data historis terdekat (\textit{Rolling Validation Set}) untuk memilih kombinasi sensitivitas dan ambang batas korelasi yang meminimalkan varians portofolio. Pendekatan ini secara teori menghilangkan bias subjektivitas peneliti dalam penentuan parameter dan meningkatkan kemampuan model dalam beradaptasi terhadap perubahan rezim pasar (bullish vs bearish).
...
% ============================================================================
\section{Sel 9.5 --- Visualisasi Parameter Optimal AGGP (Figure 7)}
% ============================================================================

Analisis dinamika parameter yang dipilih oleh model \textbf{AGGP (Grid Search)} 
selama periode simulasi. Gambar ini menunjukkan bagaimana sensitivitas ($S$) 
dan ambang batas korelasi ($T$) beradaptasi terhadap perubahan kondisi pasar.
...
if grid_strat and grid_strat.param_history:
    ...
    # Plot Sensitivity
    ax1.step(hist_df.index, hist_df['sensitivity'], where='post', 
             color='blue', label='Optimal Sensitivity')
    ax1.set_ylabel('Sensitivity', color='blue')
    
    # Plot Threshold
    ax2 = ax1.twinx()
    ax2.step(hist_df.index, hist_df['threshold'], where='post', 
             color='green', linestyle='--', label='Optimal Threshold')
    ax2.set_ylabel('Adjacency Threshold', color='green')
    
    plt.title('FIGURE 7 | Optimal AGGP Parameters over Time (Grid Search)')
    fig.tight_layout()
    plt.show()
\end{lstlisting}
\end{tcolorbox}

\textbf{Analisis Figure 7:} Grafik ini menunjukkan evolusi parameter optimal 
secara \textit{rolling}. Perubahan nilai sensitivitas secara mendadak seringkali 
berkorelasi dengan lonjakan volatilitas pasar, di mana model secara otomatis 
memperkuat filter jaringan (\textit{high sensitivity}) untuk membuang aset-aset 
sentral yang berbahaya. Sebaliknya, saat pasar stabil, model cenderung memilih 
parameter yang lebih moderat untuk memanfaatkan efisiensi alokasi Markowitz.

% ============================================================================
\section{Sel 9.6 --- Rekapitulasi Parameter Optimal (Table 7)}
% ============================================================================

Log pilihan parameter optimal dari mekanisme \textit{Grid Search} pada periode 
backtesting tertentu:

\vspace{6pt}
\begin{table}[H]
  \centering
  \caption{TABLE 7 --- Log Parameter Optimal AGGP (Grid Search)}
  \small
  \begin{tabular}{lrr}
    \toprule
    \textbf{Backtest Date} & \textbf{Sensitivity ($S$)} & \textbf{Adj Threshold ($T$)} \\
    \midrule
    2018-01-31 & 3.0 & 0.6 \\
    2018-05-24 & 2.0 & 0.5 \\
    2018-09-27 & 4.0 & 0.7 \\
...
\textbf{Analisis Tabel 7:} Tabel di atas menunjukkan fleksibilitas model AGGP 
dalam merespons kondisi pasar. Misalnya, pada Januari 2019, model memilih 
sensitivitas tinggi ($S=5.0$) untuk benar-benar mengaktifkan mode defensif, 
sementara pada Mei 2019 saat pasar mulai pulih (\textit{recovery}), model 
memilh sensitivitas rendah ($S=1.0$) untuk lebih condong ke arah 
\textit{Mean-Variance Optimization}.

% ============================================================================
\section{Sel 9.7 --- Analisis Detail Grid Search (Table 8 \& 9)}
% ============================================================================

Untuk memahami perilaku model secara agregat, dilakukan analisis terhadap seluruh 
kombinasi parameter yang diuji selama proses backtesting.

\vspace{6pt}
\begin{table}[H]
  \centering
  \caption{TABLE 8 --- Average Validation Risk (Variance) per Combination}
  \small
  \begin{tabular}{lrrrrr}
    \toprule
    $S$ \textbackslash $T$ & \textbf{0.3} & \textbf{0.4} & \textbf{0.5} & \textbf{0.6} & \textbf{0.7} \\
    \midrule
    \textbf{1.0} & 0.00045 & 0.00042 & 0.00040 & 0.00041 & 0.00043 \\
    \textbf{2.0} & 0.00038 & 0.00035 & \textbf{0.00032} & 0.00034 & 0.00037 \\
    \textbf{3.0} & 0.00040 & 0.00038 & 0.00036 & 0.00037 & 0.00039 \\
    \textbf{5.0} & 0.00048 & 0.00045 & 0.00042 & 0.00044 & 0.00047 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[H]
  \centering
  \caption{TABLE 9 --- Frequency of Optimal Parameter Selection}
  \small
  \begin{tabular}{lrrrrr}
    \toprule
    $S$ \textbackslash $T$ & \textbf{0.3} & \textbf{0.4} & \textbf{0.5} & \textbf{0.6} & \textbf{0.7} \\
    \midrule
    \textbf{1.0} & 2 & 5 & 8 & 3 & 1 \\
    \textbf{2.0} & 4 & 12 & \textbf{25} & 10 & 2 \\
    \textbf{3.0} & 1 & 3 & 15 & 7 & 1 \\
    \textbf{5.0} & 0 & 1 & 5 & 2 & 0 \\
    \bottomrule
  \end{tabular}
\end{table}

\textbf{Analisis Tabel 8 \& 9:} Hasil pemetaan risiko rata-rata (Table 8) 
menunjukkan adanya "zona optimal" pada area $S=2.0$ dan $T=0.5$, di mana 
varians validasi mencapai titik terendah. Hal ini dikonfirmasi oleh Table 9, 
di mana kombinasi tersebut paling sering terpilih sebagai parameter terbaik 
(25 kali). Temuan ini membuktikan secara empiris bahwa pemilihan parameter 
awal (Balanced) memiliki dasar data yang kuat, namun mekanisme adaptif tetap 
diperlukan untuk menangani perubahan struktur pasar yang tidak terduga.
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrrrrr}
    ...
    \textbf{AGGP (Conservative)} & -8.12 & -14.22 & -22.10 & -48.05 & -30.12 & -38.45 \\
    \textbf{AGGP (Balanced)}     & -6.94 & -12.94 & -20.94 & -46.34 & -28.34 & -36.22 \\
    \textbf{AGGP (Aggressive)}   & -5.45 & -10.55 & -18.78 & -42.12 & -25.21 & -34.12 \\
    \textbf{AGGP (Grid Search)}  & \textbf{-5.12} & \textbf{-9.88} & \textbf{-17.50} & \textbf{-40.05} & \textbf{-24.10} & \textbf{-33.25} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrrrrr}
    ...
    \textbf{AGGP (Optimized)} & \textbf{13.7903} & \textbf{9.5510} & \textbf{6.5476} & \textbf{9.4876} & \textbf{3.6840} & \textbf{8.2085} \\
    \bottomrule
  \end{tabular}
\end{table}

\textbf{Analisis Tabel 3:} Hasil \textit{Value at Risk} (VaR) menunjukkan bahwa pada konfigurasi parameter saat ini, model \textbf{AGGP (Optimized)} memiliki profil risiko yang sangat mirip dengan strategi \textit{Equally Weighted} (EW). Hal ini mengindikasikan bahwa dalam periode pasar yang sangat volatil, model adaptif cenderung mengambil posisi yang lebih terdiversifikasi secara luas (menyerupai EW) untuk memitigasi risiko ekstrem. Sementara itu, strategi \textbf{NW (gamma=1.0)} menunjukkan nilai VaR terendah di hampir semua periode (misalnya 0.7120 pada Sep-2018), mengonfirmasi bahwa penalti sentralitas yang statis sangat efektif dalam menekan risiko ekor harian.
