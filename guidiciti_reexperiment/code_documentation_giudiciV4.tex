...
\begin{enumerate}
    ...
    \item \textbf{Glasso Markowitz (GM)}: Menggunakan regularisasi \textit{Graphical Lasso} untuk menyaring *noise* matriks korelasi.
    \item \textbf{Network Markowitz (NW)}: Baseline utama dari Giudici et al. (2020) yang menggunakan \textit{Random Matrix Theory} (RMT) dan \textit{Minimum Spanning Tree} (MST). Direpresentasikan dengan $\gamma=0$ (RMT+MST murni) dan $\gamma=1.0$ (disertai penalti sentralitas).
    \item \textbf{Adaptive Graph-Gated Portfolio (AGGP)}: Model usulan (\textit{Proposed Method}) utama yang menggabungkan optimasi Markowitz (CM/GM) dengan sensor kondisi pasar dinamis berbasis Graph Density untuk melampaui NW di masa krisis maupun pemulihan.
\end{enumerate}
