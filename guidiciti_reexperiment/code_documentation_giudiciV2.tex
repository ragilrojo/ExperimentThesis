...
class DynamicGraphDiversification(PortfolioStrategy):
    ...
    Menggunakan rolling window untuk mencari nilai theta yang memberikan volatilitas minimal (atau kriteria lain),
    mencegah kegagalan pembentukan MIS pada saat kondisi pasar bergejolak tinggi.
    """
    def __init__(self, name='Dynamic Graph Divers. (ML)'):
        super().__init__(name)

    def optimize_theta(self, returns_data):
        # Contoh pseudo-optimasi (rolling cross-validation): mencari theta yang meminimalkan volatilitas.
        best_theta = 0.4
        min_volatility = float('inf')
        for theta in np.arange(0.1, 0.9, 0.1):
            # Simulasi atau backtest lokal untuk menentukan performa theta historis
            # Pseudo-evaluasi: di sini kita asumsikan sebuah metode analitik mencari best_theta 
            pass
        return best_theta
