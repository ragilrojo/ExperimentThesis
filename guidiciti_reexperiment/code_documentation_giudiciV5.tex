...
\begin{enumerate}
    ...
    \item \textbf{Adaptive Graph-Gated Portfolio (AGGP)}: Model usulan (\textit{Proposed Method}) yang menggabungkan optimasi Markowitz (\textit{Global Minimum Variance}) dengan komponen Network Markowitz ($\gamma=1.0$). Proporsi campuran ($\gamma_t$) bersifat \textbf{adaptif}: dikendalikan secara otomatis oleh sensor rata-rata korelasi parsial (\textit{average partial correlation}) yang diekstrak dari matriks presisi Graphical Lasso. Saat pasar stres (korelasi tinggi), AGGP otomatis bergeser ke mode Markowitz yang lebih defensif; saat korelasi rendah (pasar normal), kembali memprioritaskan komponen Network yang agresif.
\end{enumerate}
...
%=============================================================================
Setiap strategi diatur dalam kelas yang mewarisi \texttt{PortfolioStrategy}. Implementasi \textit{Network Markowitz} mencakup penalti sentralitas melalui parameter $\gamma$. Kelas \textbf{AGGP} (\textit{Adaptive Graph-Gated Portfolio}) merupakan model usulan yang telah diperbarui dengan empat perbaikan kunci:

\begin{enumerate}
    \item \textbf{[Fix 1] Sensor yang lebih sensitif:} Mengganti \textit{graph density} berbasis threshold biner ($>0.5$) dengan \textit{average partial correlation} dari matriks presisi Graphical Lasso. Sensor ini jauh lebih responsif terhadap perubahan kondisi pasar yang halus.
    \item \textbf{[Fix 2] Logika gamma dibalik (defensif saat krisis):} Semakin tinggi rata-rata korelasi parsial (kondisi stres pasar), nilai $\gamma_t$ semakin \textit{kecil}, sehingga portofolio lebih condong ke Markowitz (risk-minimizing). Ini kebalikan dari implementasi sebelumnya yang justru meningkatkan bobot porsi grafik saat kondisi pasar tertekan.
    \item \textbf{[Fix 3] Komponen \texttt{w\_net} menggunakan formulasi NW terbukti:} Mengganti bobot $1/\text{centrality}$ (yang berperilaku seperti EW) dengan hasil optimasi \textit{Network Markowitz} pada $\gamma=1.0$ --- nilai yang terbukti sebagai best performer dari Giudici et al. (2020).
    \item \textbf{[Fix 4] Fallback aman dengan \textit{return constraint}:} Menambahkan constraint return minimum ($\mathbf{w}^\top\boldsymbol{\mu} \geq \bar{\mu}$) pada Markowitz dan menyediakan fallback ke EW jika optimizer gagal.
\end{enumerate}

\begin{lstlisting}[caption={Kelas-kelas strategi portofolio (AGGP v2 -- 4 Fixes Applied)}]
...
# ── 8. Adaptive Graph-Gated Portfolio (AGGP) v2 ────────────────────────────────
class AdaptiveGraphPortfolio(PortfolioStrategy):
    ...
    Mekanisme adaptasi gamma_t (FIX 1 & 2):
      - avg_corr tinggi (pasar stres, korelasi meningkat) -> gamma_t kecil
        -> portofolio lebih ke Markowitz (defensif, minimasi varians).
      - avg_corr rendah (pasar normal) -> gamma_t besar
        -> portofolio lebih ke Network Markowitz (agresif, diversifikasi graph).

    Komponen w_net (FIX 3):
      - Menggunakan formulasi NW gamma=1.0 (best performer dari Giudici et al.)
        bukan 1/centrality yang berperilaku seperti EW.

    Fallback aman (FIX 4):
      - Jika optimizer gagal, kembali ke EW (bukan bobot berpotensi salah).
    """
    def __init__(self, name="AGGP (Optimized)", sensitivity=1.5):
        super().__init__(name)
        self.sensitivity = sensitivity  # Skalar sensitivitas sensor avg_corr
    ...
    def get_weights(self, returns_data):
        ...
        try:
            ...
            glasso.fit(returns_data.values)
            Sf = glasso.covariance_     # Matriks kovarians ter-sparsifikasi
            Pf = glasso.precision_      # Matriks presisi (inverse kovarians)
        except Exception:
            # Fallback: RMT filter jika Glasso tidak konvergen
            Cf = apply_rmt_filter(returns_data)
            Sf = np.outer(sig, sig) * Cf
            Pf = Cf  # Approx precision dengan korelasi RMT

        # --- FIX 1: Sensor avg_corr dari matriks presisi (lebih sensitif) ---
        # Rata-rata elemen off-diagonal |Pf| sebagai proxy tekanan pasar
        mask    = ~np.eye(n, dtype=bool)
        avg_corr = np.mean(np.abs(Pf[mask]))

        # --- FIX 2: Logika gamma dibalik (defensif saat korelasi tinggi) ---
        # avg_corr tinggi -> gamma_t kecil -> lebih ke Markowitz (risk-min)
        gamma_t = np.clip(1.0 - avg_corr * self.sensitivity, 0.1, 0.9)

        # --- FIX 3: w_net = Network Markowitz gamma=1.0 (bukan 1/centrality) ---
        ...
        w_net = res_nw.x if res_nw.success else np.ones(n) / n  # FIX 4 (partial)

        # --- FIX 4: Markowitz GMV dengan return constraint & fallback aman ---
        obj_mko  = lambda w: w @ Sf @ w
        cons_mko = [
            {'type': 'eq',   'fun': lambda w: np.sum(w) - 1},
            {'type': 'ineq', 'fun': lambda w: w @ mu - mu.mean()}  # return >= mean
        ]
        res_mko = minimize(obj_mko, np.ones(n) / n, method='SLSQP',
                           bounds=[(0, 1)] * n, constraints=cons_mko)
        w_mko = res_mko.x if res_mko.success else np.ones(n) / n  # fallback EW

        # --- Blending adaptif: (1-gamma_t)*Markowitz + gamma_t*NetworkMarkowitz ---
...
gamma_values = [1.0] # Memilih gamma 1.0 sebagai best performer Giudici
strategies = [
...
%=============================================================================
Untuk menguji kekokohan strategi di berbagai kondisi pasar, dilakukan pemetaan performa ke dalam tiga fase utama: \textbf{Bearish} (Januari 2018 -- Maret 2019), \textbf{Recovery} (April 2019 -- Juni 2019), dan \textbf{Stable/Sideways} (Juli 2019 -- Oktober 2019). Analisis ini bertujuan untuk membuktikan bahwa model \textbf{AGGP v2} mampu beradaptasi sebagai \textit{"All-Terrain Model"}:
\begin{itemize}
    \item Saat \textbf{Bearish}: sensor \textit{avg\_corr} mendeteksi korelasi parsial tinggi $\Rightarrow$ $\gamma_t$ mengecil $\Rightarrow$ AGGP lebih ke Markowitz (defensif, minimasi varians).
    \item Saat \textbf{Recovery/Stable}: korelasi parsial menurun $\Rightarrow$ $\gamma_t$ membesar $\Rightarrow$ AGGP lebih ke Network Markowitz $\gamma=1.0$ (agresif, memanfaatkan diversifikasi graph).
\end{itemize}
Hipotesis yang diuji: AGGP v2 seharusnya mengalahkan NW($\gamma=1.0$) minimal pada fase \textit{Bearish} dan \textit{Stable}, karena kemampuan adaptifnya menghindari under-diversifikasi di kondisi korelasi tinggi.
